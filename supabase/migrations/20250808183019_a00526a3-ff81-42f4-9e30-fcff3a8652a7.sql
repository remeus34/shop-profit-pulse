...
-- Expense categories table (supports parent/child via self-reference)
CREATE TABLE IF NOT EXISTS public.expense_categories (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  user_id UUID NOT NULL,
  name TEXT NOT NULL,
  parent_id UUID NULL REFERENCES public.expense_categories(id) ON DELETE CASCADE,
...
-- Expense items (products under a child category)
...
-- Expense variants (size/color/sku-level costs)
CREATE TABLE IF NOT EXISTS public.expense_variants (
  ...
  created_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now(),
  CONSTRAINT expense_variants_unique UNIQUE (item_id, size, COALESCE(color, ''), COALESCE(sku, ''))
);
