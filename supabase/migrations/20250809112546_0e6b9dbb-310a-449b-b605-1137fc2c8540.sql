-- 1) Create table for shipping labels with persistence and linking to orders
CREATE TABLE IF NOT EXISTS public.shipping_labels (
  ...
  ship_date_date date GENERATED ALWAYS AS (COALESCE(ship_date::date, now()::date)) STORED,
  to_name text NULL,
  address1 text NULL,
  city text NULL,
  state text NULL,
...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'shipping_labels' AND policyname = 'Shipping Labels: Select own'
  ) THEN
    CREATE POLICY "Shipping Labels: Select own" ON public.shipping_labels
    FOR SELECT USING (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'shipping_labels' AND policyname = 'Shipping Labels: Insert own'
  ) THEN
    CREATE POLICY "Shipping Labels: Insert own" ON public.shipping_labels
    FOR INSERT WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'shipping_labels' AND policyname = 'Shipping Labels: Update own'
  ) THEN
    CREATE POLICY "Shipping Labels: Update own" ON public.shipping_labels
    FOR UPDATE USING (auth.uid() = user_id) WITH CHECK (auth.uid() = user_id);
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'shipping_labels' AND policyname = 'Shipping Labels: Delete own'
  ) THEN
    CREATE POLICY "Shipping Labels: Delete own" ON public.shipping_labels
...
-- 1) When label_id is present, enforce uniqueness per user_id + label_id
...
-- 2) When label_id is missing, fall back to (user_id, tracking, ship_date_date, amount)
...
-- Function to recalc order external shipping cost from labels
...
-- Trigger to keep orders in sync on insert/update/delete of shipping labels
...
-- Auto-match attempt: find order id in reference/notes within ±7 days and pick closest by date
...
-- Trigger to auto-match after insert
