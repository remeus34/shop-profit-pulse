-- Retry 2: avoid generated column, use trigger-populated date column for uniqueness
...
-- triggers to maintain ship_date_date and updated_at
CREATE OR REPLACE FUNCTION public.set_shipping_labels_derived()
RETURNS trigger AS $$
BEGIN
  IF NEW.ship_date IS NOT NULL THEN
    NEW.ship_date_date := (NEW.ship_date AT TIME ZONE 'UTC')::date;
  ELSE
    NEW.ship_date_date := NULL;
  END IF;
  NEW.updated_at := now();
  RETURN NEW;
